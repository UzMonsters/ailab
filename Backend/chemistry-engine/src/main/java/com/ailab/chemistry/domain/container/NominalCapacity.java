...
public record NominalCapacity(Volume volume) {
    public NominalCapacity {
        Objects.requireNonNull(volume, "volume must not be null");
        if (volume.in(VolumeUnit.LITER).compareTo(BigDecimal.ZERO) <= 0) {
            throw new ContainerException(ContainerErrorCode.INVALID_PROFILE, "Nominal capacity must be positive");
        }
    }
}
