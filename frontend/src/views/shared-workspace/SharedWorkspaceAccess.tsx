...
export default function SharedWorkspaceAccess({ token, locale }: { token: string; locale: string }) {
  const ts = useTranslations("sharedWorkspace");
  ...
  async function openShare() {
    ...
    try {
      ...
      const message = response.status === 410
        ? ts("error.expired")
        : response.status === 401
          ? ts("error.passwordRequired")
          : response.status === 403
            ? ts("error.wrongPassword")
            : response.status === 404
              ? ts("error.notFound")
              : response.message || ts("error.generic");
    ...
    <section className="relative mx-auto grid min-h-[calc(100vh-5rem)] max-w-5xl place-items-center">
      <div className="grid w-full overflow-hidden rounded-3xl border border-white/10 bg-[#0d111c]/95 shadow-2xl lg:grid-cols-[1.1fr_.9fr]">
        <div className="relative min-h-72 bg-[#090d16] p-6">
          {preview ? <Image unoptimized width={960} height={540} src={preview} alt={ts("previewAlt", { name: workspace?.name ?? '' })} className="h-full max-h-[540px] w-full rounded-2xl object-cover" /> : <div className="grid h-full min-h-72 place-items-center rounded-2xl border border-dashed border-white/10 bg-[linear-gradient(rgba(148,163,184,.08)_1px,transparent_1px),linear-gradient(90deg,rgba(148,163,184,.08)_1px,transparent_1px)] bg-[size:24px_24px]"><div className="text-center text-slate-400"><FlaskConical className="mx-auto mb-3 text-violet-400" size={46}/><p>{workspace ? ts("noPreviewAvailable") : ts("previewAfterVerify")}</p></div></div>}
        </div>
        <div className="flex flex-col justify-center p-7 sm:p-10">
          ...
          <p className="text-xs font-semibold uppercase tracking-[.18em] text-violet-300">{ts("brand")}</p>
          <h1 className="mt-2 text-3xl font-bold">{workspace?.name ?? ts("defaultTitle")}</h1>
          <p className="mt-3 text-sm leading-6 text-slate-400">{workspace ? `${workspace.science} · ${workspace.role.toLowerCase()} ${ts("access")}` : ts("verifyDesc")}</p>
          {!workspace && <form className="mt-8 space-y-3" onSubmit={(event)=>{event.preventDefault();void openShare()}}>
            <label className="block text-sm font-medium text-slate-200" htmlFor="share-password">{ts("password")} <span className="font-normal text-slate-500">({ts("ifRequired")})</span></label>
            <div className="relative"><KeyRound className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={17}/><input id="share-password" type="password" autoComplete="current-password" value={password} onChange={(event)=>setPassword(event.target.value)} className="w-full rounded-xl border border-white/10 bg-black/25 py-3 pl-10 pr-4 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-500/25" placeholder={ts("enterPassword")} /></div>
          ...
          {workspace && <div className="mt-7 space-y-4">
            ...
            <Link href={`/${locale}/workspace/sandbox?workspace=${encodeURIComponent(workspace.workspaceId)}&shared=1${typeof window !== 'undefined' ? window.location.hash : ''}`} className="flex w-full items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 py-3 font-semibold transition hover:bg-violet-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-300">{ts("openWorkspace")} <ArrowRight size={18}/></Link>
          </div>}
          <Link href={`/${locale}`} className="mt-6 text-center text-xs text-slate-500 hover:text-slate-300">{ts("returnToBrand")}</Link>
        </div>
      </div>
    </section>
  </main>;
