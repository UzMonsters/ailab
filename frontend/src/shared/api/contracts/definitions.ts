...
export interface SandboxEventCommand {
...
export interface WorkspaceEventAck {
  clientEventId: string;
  eventId: string;
  eventType: string;
  workspaceId: string;
...
export interface EquipmentDetails extends EquipmentSummary {
  datasetId: string;
  capabilities: string[];
  ports: Array<{ id: string; name: string; type: string; direction: string }>;
}
...
export interface MaterialSummary {
  ...
  name: string;
  formula: string;
  category: 'COMPOUND' | 'ELEMENT' | string;
  phase: string;
  molarMass?: number;
}
