...
export function LearningPanel({ tab }: { tab: string }) {
  ...
  return <div className="space-y-5">
    <div className="rounded-2xl border border-violet-400/15 bg-gradient-to-r from-violet-500/15 via-[#0b101a] to-cyan-500/10 p-6 md:p-8">
      <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><div className="mb-2 text-xs font-bold uppercase tracking-[.18em] text-violet-400">Curriculum workspace</div><h2 className="text-2xl font-bold">{data.title}</h2><p className="mt-2 max-w-2xl text-sm text-[#8490a3]">{data.subtitle}</p></div>{tab === 'levels' && <Link href="/en/admin/learning/levels" className="inline-flex items-center gap-2 rounded-lg bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white">Open level manager <ArrowUpRight size={16}/></Link>}</div>
    </div>
