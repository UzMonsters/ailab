...
export function MaterialsPanel() {
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      setLoading(true);
      try {
        const res = await adminPlatformApi.materials.list({ size: 50, ...(search ? { q: search } : {}) });
        setItems(res.items || []);
      } catch { setItems([]); }
      setLoading(false);
    }, 300);
    ...
    <div className="space-y-3">
      ...
      {!loading && items.length === 0 && (
        <div className="py-8 text-center text-xs text-slate-500">
          <Atom size={24} className="mx-auto mb-2 text-slate-600" />
      ...
      <div className="space-y-1.5">
        {items.map(item => (
          <div
            key={String(item.id)}
            draggable
            onDragStart={e => handleDragStart(e, 'MATERIAL_REFERENCE', { materialId: item.id })}
          ...
          >
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded bg-cyan-600/10">
              <Atom size={14} className="text-cyan-400" />
            </div>
            <div className="min-w-0">
              <p className="text-xs font-medium text-slate-200 truncate">
                {String(obj(item.translations).en?.name || obj(item.translations).ru?.name || item.name || item.code || 'Material')}
              </p>
              <p className="text-[10px] text-slate-500">{String(item.formula || item.kind || '')}</p>
            </div>
          </div>
        ))}
      </div>
