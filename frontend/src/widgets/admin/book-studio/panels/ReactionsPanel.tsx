...
export function ReactionsPanel() {
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      setLoading(true);
      try {
        const res = await adminPlatformApi.chemistry.reactions.list({ size: 50, ...(search ? { q: search } : {}) });
        setItems(res.items || []);
      } catch { setItems([]); }
      setLoading(false);
    }, 300);
      ...
      <div className="space-y-1.5">
        {items.map(item => (
          <div
            key={String(item.id)}
            draggable
            onDragStart={e => handleDragStart(e, 'REACTION_REFERENCE', { reactionId: item.id })}
          ...
          >
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded bg-emerald-600/10">
              <Sparkles size={14} className="text-emerald-400" />
            </div>
            <div className="min-w-0">
              <p className="text-xs font-medium text-slate-200 truncate">
                {String(obj(item.translations).en?.name || obj(item.translations).ru?.name || item.name || item.code || 'Reaction')}
              </p>
              <p className="text-[10px] text-slate-500 truncate">{String(item.equation || '')}</p>
            </div>
          </div>
        ))}
      </div>
