...
type Props = { content: string; onChange: (html: string) => void; onEditStart?: () => void; onEditEnd?: () => void; hideToolbar?: boolean };

export function RichTextEditor({ content, onChange, onEditStart, onEditEnd, hideToolbar = false }: Props) {
  ...
  return <div className={`space-y-2 ${hideToolbar ? 'h-full flex flex-col' : ''}`}>
    {!hideToolbar && (
    ...
    {!hideToolbar && linkOpen && <form className="flex gap-2 rounded-lg border border-violet-400/30 bg-[#111827] p-2" onSubmit={event => { event.preventDefault(); applyLink(); }}><label className="sr-only" htmlFor="book-link-url">Link URL</label><input id="book-link-url" autoFocus value={linkUrl} onChange={event => setLinkUrl(event.target.value)} className="min-w-0 flex-1 rounded-md border border-white/10 bg-black/20 px-3 py-1.5 text-xs text-white" placeholder="https://example.com"/><button type="submit" className={control}>Apply</button><button type="button" className={control} onClick={() => applyLink('')}>Remove</button><button type="button" className={control} onClick={() => setLinkOpen(false)}>Cancel</button></form>}
    <EditorContent editor={editor} className={`${hideToolbar ? 'flex-1 h-full' : 'min-h-44 rounded-lg border border-white/10 bg-[#fffdf5] p-4'} text-sm leading-6 outline-none focus-within:border-violet-500 [&_.tiptap]:min-h-36 [&_.tiptap]:outline-none [&_h1]:text-3xl [&_h1]:font-bold [&_h2]:text-2xl [&_h2]:font-bold [&_h3]:text-xl [&_h3]:font-bold [&_ol]:list-decimal [&_ol]:pl-5 [&_ul]:list-disc [&_ul]:pl-5 [&_table]:w-full [&_table]:border-collapse [&_td]:border [&_td]:border-slate-300 [&_td]:p-1 [&_th]:border [&_th]:border-slate-400 [&_th]:bg-slate-100 [&_th]:p-1 ${hideToolbar ? '[&_.tiptap]:h-full [&_.tiptap]:w-full text-[inherit]' : 'text-slate-950'}`}/>
  </div>;
}
