  ...
  return <div className="min-h-screen bg-[#070b14] p-4 text-white md:p-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
      ...
      <main className="min-h-[620px] overflow-hidden rounded-2xl border border-white/[.06] bg-[#0b101a]">
        ...
        {activeTab === 'Localization' && <div className="p-6"><div className="grid gap-4 md:grid-cols-3">{[['RU','Hydrogen','Complete'],['UZ','Hydrogen','Review'],['EN','Hydrogen','Complete']].map(([lang,name,status]) => <section key={lang} className="rounded-xl border border-white/[.06] bg-[#141b2a] p-4"><div className="flex justify-between text-xs"><strong>{lang}</strong><span className={status === 'Complete' ? 'text-emerald-400' : 'text-amber-400'}>{status}</span></div><input className={inputClass} defaultValue={name}/><textarea rows={4} className={`${inputClass} resize-none`} placeholder="Localized description"/></section>)}</div></div>}
