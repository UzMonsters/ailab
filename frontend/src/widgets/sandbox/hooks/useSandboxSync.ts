...
function thumbnailData(engine: Engine): string {
  ...
  const labels = items.slice(0, 12).map((item, index) => {
    ...
    return `<g><rect x="${x}" y="${y}" width="132" height="88" rx="16" fill="#172033" stroke="#8b5cf6" stroke-width="2"/><path d="M${x + 44} ${y + 18}h44v18l16 32a16 16 0 0 1-14 24H${x + 42}a16 16 0 0 1-14-24l16-32z" fill="#94e6ff" fill-opacity=".38" stroke="#e5e7eb" stroke-width="3"/><text x="${x + 66}" y="${y + 116}" text-anchor="middle" fill="#e5e7eb" font-size="13" font-family="Arial">${name.slice(0, 20)}</text></g>`;
  }).join('');
