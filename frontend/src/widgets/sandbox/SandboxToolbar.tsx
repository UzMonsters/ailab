...
export function SandboxToolbar({
  ...
  if (collapsed) return <button type="button" aria-label={ts("toolbar.showToolbar")} title={ts("toolbar.showToolbar")} onPointerDown={event => event.stopPropagation()} onClick={() => setCollapsed(false)} className={`absolute left-1/2 ${showNavbar ? 'top-14' : 'top-0'} z-[60] grid h-7 w-10 -translate-x-1/2 place-items-center rounded-b-lg border border-t-0 border-border bg-card/95 text-muted-foreground shadow-md transition-colors hover:text-foreground`}><ChevronDown size={17} /></button>;
  return (
    ...
    <div className="sandbox-toolbar sandbox-toolbar-reveal flex max-w-[calc(100vw-1rem)] items-center gap-1.5 overflow-x-auto overflow-y-visible rounded-full border border-border bg-card/95 p-1.5 shadow-[0_8px_32px_rgba(139,92,246,0.12)] backdrop-blur-xl transition-all duration-300 hover:shadow-[0_8px_32px_rgba(139,92,246,0.22)] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <ToolButton label={ts("toolbar.undo")} disabled={!canUndo} onClick={onUndo}><Undo2 size={17} /></ToolButton>
      <ToolButton label={ts("toolbar.redo")} disabled={!canRedo} onClick={onRedo}><Redo2 size={17} /></ToolButton>
      ...
      <div className="flex h-10 shrink-0 items-center gap-1 rounded-xl bg-foreground/5 px-1">
        <ToolButton label={ts("toolbar.zoomOut")} onClick={() => setZoom((value: number) => Math.max(0.5, value - 0.1))}><Minus size={15} /></ToolButton>
        <span className="min-w-12 text-center text-[11px] font-semibold text-foreground/80">{Math.round(zoom * 100)}%</span>
        <ToolButton label={ts("toolbar.zoomIn")} onClick={() => setZoom((value: number) => Math.min(2, value + 0.1))}><Plus size={15} /></ToolButton>
        <div className="mx-0.5 h-4 w-px bg-[var(--border)]" />
        <ToolButton label={ts("toolbar.centerScene")} onClick={() => {
          setZoom(() => 1);
        ...
        <ToolButton label={ts("toolbar.fitView")} onClick={() => {
      ...
      <div className="mx-1.5 h-6 w-px bg-[var(--border)]" />
      <ToolButton label={ts("toolbar.grid")} active={showGrid} onClick={() => setShowGrid(!showGrid)}><Grid3X3 size={17} /></ToolButton>
      <ToolButton label={ts("toolbar.library")} active={leftPanelVisible} onClick={() => window.innerWidth < 1600 ? setMobilePanel('library') : setLeftPanelVisible(!leftPanelVisible)}><PanelLeft size={17} /></ToolButton>
      <ToolButton label={ts("toolbar.inspector")} active={rightPanelVisible} onClick={() => window.innerWidth < 1600 ? setMobilePanel('inspector') : setRightPanelVisible(!rightPanelVisible)}><SlidersHorizontal size={17} /></ToolButton>
      <div className="mx-1.5 h-6 w-px bg-[var(--border)]" />
      ...
      <div className="flex shrink-0 gap-1.5 min-[1600px]:hidden">
        <ToolButton label={ts("toolbar.measurements")} onClick={() => { setBottomDockTab('Measurements'); setMobilePanel('dock'); }}><BarChart3 size={17} /></ToolButton>
      </div>
      
    </div>
    <button type="button" aria-label={ts("toolbar.hideToolbar")} title={ts("toolbar.hideToolbar")} onPointerDown={event => event.stopPropagation()} onClick={() => setCollapsed(true)} className="absolute left-1/2 top-full z-50 grid h-7 w-10 -translate-x-1/2 place-items-center rounded-b-lg border border-t-0 border-border bg-card/95 text-muted-foreground/70 shadow-md transition-colors hover:text-foreground"><ChevronUp size={16} /></button>
    </div>
  );
}
