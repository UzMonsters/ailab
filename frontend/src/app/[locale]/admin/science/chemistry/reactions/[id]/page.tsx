...
export default function ReactionPage({ params }: { params: { id: string } }) {
  const router = useRouter();
  const locale = useLocale();
  const isNew = params.id === 'new';
  
  const reaction = !isNew ? mockReactions.find(r => r.id === params.id) : null;
  const [activeTab, setActiveTab] = useState(REACTION_TABS[0]);

  if (!isNew && !reaction) {
    return (
      <div className="p-6 h-full flex flex-col items-center justify-center text-center bg-[#05080f]">
        <div className="bg-[#141b2a] border border-[rgba(255,255,255,0.05)] rounded-xl p-8 max-w-md w-full">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Reaction not found</h2>
          <p className="text-[#a9a5b8] mb-8">
            The reaction <span className="text-white font-mono">{params.id}</span> does not exist or has been removed from the registry.
          </p>
          <button 
            onClick={() => router.push(`/${locale}/admin/science/chemistry`)}
            className="w-full px-4 py-3 rounded-lg text-sm font-medium text-white bg-[#8b5cf6] hover:bg-[#7c3aed] transition-colors shadow-lg shadow-[#8b5cf6]/20"
          >
            Back to Chemistry
          </button>
        </div>
  ...
  return (
    <div className="p-6 h-full flex flex-col">
      <AdminPageHeader 
        title={isNew ? "Create Reaction" : `Edit Reaction: ${reaction?.name || params.id}`}
