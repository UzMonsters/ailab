...
function electronShells(count: number): number[] {
  const shells: number[] = []
  let remaining = count
  let level = 1
  while (remaining > 0) {
    const capacity = 2 * level * level
...
export default function ScientificCore({ size = 620, accentColor = '#7c3aed', electrons }: { size?: number; accentColor?: string; electrons?: number }) {
  ...
  const handlePointerMove = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (!rect || !nucleusRef.current) return
    const x = (event.clientX - rect.left) / rect.width - .5
    const y = (event.clientY - rect.top) / rect.height - .5
    nucleusRef.current.style.transform = `translate(${x * size * .012}px, ${y * size * .012}px)`
  }
  const handlePointerLeave = () => {
    if (nucleusRef.current) nucleusRef.current.style.transform = 'translate(0, 0)'
  }
  const handleClick = () => {
    const wave = Date.now()
    setWaves((current) => [...current, wave])
    window.setTimeout(() => setWaves((current) => current.filter((item) => item !== wave)), 1500)
  }
  ...
  const shells = electrons ? electronShells(electrons) : []
  const renderShell = (count: number, level: number) => {
    const rx = size * (.16 + level * .055)
    const ry = rx * .42
    const rotate = 6 + level * 31
    const dur = 9 + level * 1.8
    const path = pathForRing(rx, ry)
    return <g key={`shell-${level}`} transform={`rotate(${rotate} ${center} ${center})`} className="scientific-shell">
      <ellipse cx={center} cy={center} rx={rx} ry={ry} />
      {Array.from({ length: count }, (_, index) => <circle key={index} r={size * .011} fill={index % 3 === 0 ? '#fff' : accentColor}><animateMotion dur={`${dur}s`} begin={`${-(index / count) * dur}s`} repeatCount="indefinite" path={path} /></circle>)}
    </g>
  }
  const nucleus = <g ref={nucleusRef} className="scientific-nucleus"><circle cx={center} cy={center} r={size * .3} fill={`url(#${id}-nucleus-outer)`} filter={`url(#${id}-blur)`} /><circle cx={center} cy={center} r={size * .14} fill={`url(#${id}-nucleus-core)`} filter={`url(#${id}-blur)`}><animate attributeName="r" values={`${size * .12};${size * .155};${size * .12}`} dur="7s" repeatCount="indefinite" /></circle><circle cx={center} cy={center} r={size * .038} fill={`url(#${id}-nucleus-core)`}><animate attributeName="r" values={`${size * .034};${size * .044};${size * .034}`} dur="7s" repeatCount="indefinite" /></circle>{waves.map((wave) => <circle key={wave} cx={center} cy={center} r={size * .04} fill="none" stroke="#fff" strokeWidth={size * .002}><animate attributeName="r" values={`${size * .04};${size * .5}`} dur="1.4s" fill="freeze" /><animate attributeName="opacity" values=".9;0" dur="1.4s" fill="freeze" /></circle>)}</g>
  return <div className="scientific-core-model" aria-hidden="true">
    <svg ref={svgRef} viewBox={`0 0 ${size} ${size}`} width="100%" height="100%" onPointerMove={handlePointerMove} onPointerLeave={handlePointerLeave} onClick={handleClick}>
      <defs>
        <radialGradient id={`${id}-nucleus-outer`}><stop stopColor={accentColor} stopOpacity=".58" /><stop offset=".45" stopColor={accentColor} stopOpacity=".25" /><stop offset="1" stopColor={accentColor} stopOpacity="0" /></radialGradient>
